...
export function UserDetailDialog({ open, onClose, user: initialUser }: UserDetailDialogProps) {
  ...
  const loadUserData = async () => {
    ...
    try {
      const [enrollmentsData, attemptsData] = await Promise.all([
        api.enrollments.getByUserId(initialUser.id),
        api.quizAttempts.getByUserId(initialUser.id),
      ])
  ...
  const avgProgress =
    enrollments.length > 0 ? enrollments.reduce((sum, e) => sum + e.progress_percentage, 0) / enrollments.length : 0
                ...
                <div className="grid grid-cols-3 gap-4 text-sm">
                  ...
                  {user.lockout_until && (
                    <div>
                      ...
                      <p className="font-medium text-orange-500">{new Date(user.lockout_until).toLocaleDateString()}</p>
                    </div>
                  )}
                  {user.deleted_at && (
                    <div>
                      ...
                      <p className="font-medium text-red-500">{new Date(user.deleted_at).toLocaleDateString()}</p>
                    </div>
                  )}
                </div>
              </Card>
